...
- (void)reversalMode{
    for (int i=0; i<self.subviews.count; i++) {
        ...
        // animation
        [[subview rac_signalForSelector:@selector(touchesBegan:withEvent:)] subscribeNext:^(id x) {
            [UIView animateWithDuration:1.2f delay:0.0f usingSpringWithDamping:0.4f initialSpringVelocity:0.3f options:UIViewAnimationOptionCurveEaseInOut animations:^{
